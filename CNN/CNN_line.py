...
def get_CNN():
    ...
    sgd = keras.optimizers.SGD(lr=0.1, momentum=0.9, decay=0.005)
...
def run_cnn_linear():
    ...
    train, label = load_train(col, '/media/meng/9079-7B0D/clean_data/train/', 4)
    train, label = shuffle_data(train, label)
    label = label
    train = train.reshape(train.shape[0], 200, 13, 1)
    hist = model.fit(train, label, batch_size=200, epochs=5, callbacks=[early_stopping], verbose=1, shuffle=True, validation_data=(test, test_y))
    #hists.append(hist)
    json_string = model.to_json()
    open('/home/meng/PyProject/Motor-life-prediction/CNN/model.json','w').write(json_string) 
    model.save_weights('/home/meng/PyProject/Motor-life-prediction/CNN/model.h5')
    #figure_out(hists)
